...
<link rel="icon" type="image/svg+xml" href="data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'%3E%3Ctext y='.9em' font-size='90'%3E%F0%9F%9B%A1%3C/text%3E%3C/svg%3E">
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .sg-tab-info{display:inline-block;padding:.05em .4em;border-radius:10px;font-size:.6rem;font-weight:600;color:var(--color-muted);background:transparent;border:1px dashed var(--color-border);font-variant-numeric:tabular-nums}
  .sg-info-row td{opacity:.75}
  .sg-info-reason{display:inline-block;font-size:.65rem;font-weight:600;letter-spacing:.04em;text-transform:uppercase;padding:.1rem .45rem;border-radius:999px;background:var(--color-surface);border:1px solid var(--color-border);color:var(--color-muted)}
  .sg-details{margin-top:1rem;border:1px solid var(--color-border);border-radius:calc(var(--radius)*1.5);background:var(--color-surface);overflow:hidden}
  .sg-details>summary{display:flex;align-items:center;gap:.6rem;padding:.85rem 1rem;cursor:pointer;list-style:none;user-select:none;font-size:.85rem;color:var(--color-text-secondary);transition:background .12s}
  .sg-details>summary::-webkit-details-marker{display:none}
  .sg-details>summary::before{content:"▸";color:var(--color-muted);font-size:.8rem;transition:transform .15s ease;display:inline-block;width:.8rem}
  .sg-details[open]>summary::before{transform:rotate(90deg)}
  .sg-details>summary:hover{background:var(--color-bg)}
  .sg-details-label{font-weight:700;color:var(--color-text);text-transform:uppercase;letter-spacing:.06em;font-size:.7rem}
  .sg-details-count{font-size:.75rem;font-weight:700;color:var(--color-muted);background:var(--color-bg);border:1px solid var(--color-border);border-radius:10px;padding:.05em .55em;font-variant-numeric:tabular-nums}
  .sg-details-hint{font-size:.7rem;color:var(--color-muted);margin-left:auto;font-style:italic}
  .sg-details-body{padding:0 1rem 1rem;border-top:1px solid var(--color-border)}
  .sg-details-body code{font-family:var(--font-mono);font-size:.85em;padding:.05em .35em;background:var(--color-bg);border-radius:3px}
  ...
  <main class="sg-main">
    ...
    <section id="findings">
      <div class="sg-section-head">
        ...
        <div class="sg-section-meta">${e(actionableAll.length)} actionable${informationalAll.length ? ` · ${e(informationalAll.length)} informational` : ""}${suppressionsCount ? ` · ${e(suppressionsCount)} suppressed` : ""}</div>
      </div>
      <div class="sg-tabs">
        ${tabInputs}
