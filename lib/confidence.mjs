...
/** Detect a Trivy base-image CVE finding regardless of how the runner labeled it. */
function isTrivyImageFinding(finding) {
  if (finding.tool === "trivyImage") return true;
  if (finding.tool === "trivy" && finding.scanMode === "image") return true;
  if (typeof finding.signature === "string" && finding.signature.startsWith("trivy-image:")) return true;
  return false;
}

/**
 * Classify a finding's confidence under the active profile.
 * @param {object} finding — { tool, severity, signature, image, type, scanMode, ... }
...
export function getConfidence(finding, profile) {
  ...
  if (sev === "UNKNOWN") return "informational";

  // License findings (Trivy SPDX flagging of every dep) are governance/compliance
  // signal, not security threat. Always demote — license posture is legitimate
  // but doesn't belong in the actionable security pane.
  if (finding.type === "license") return "informational";
  ...
  if (isTrivyImageFinding(finding) && (sev === "LOW" || sev === "MEDIUM")) {
...
export function informationalReason(finding) {
  ...
  if (finding.type === "license") return "license posture";
