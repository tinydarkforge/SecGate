...
import { runToolAsync, toolExists, normalizeSeverity, matchPattern, matchesAny } from "./utils.mjs";
...
export async function runGitleaks(target, config, addFinding, debugFn) {
  ...
  const out = await runToolAsync("gitleaks", ["detect", "--source", target, "--report-format", "json"]);
...
export async function runSemgrep(target, config, addFinding, debugFn) {
  ...
  const out = await runToolAsync("semgrep", semgrepArgs);
...
export async function runOsvScanner(target, config, addFinding, debugFn) {
  ...
  const out = await runToolAsync("osv-scanner", ["--format", "json", "-r", target]);
...
export async function runTrivy(target, config, addFinding, debugFn) {
  ...
  const out = await runToolAsync("trivy", [
    "fs",
    "--quiet",
    "--format", "json",
...
export async function runTrivyImage(target, config, addFinding, debugFn) {
  if (config.scanners.trivy === false) {
    return { status: "skipped", skipReason: "disabled in config" };
  }
  if (!toolExists("trivy")) {
  ...
  const dockerfiles = findDockerfiles(target).filter(f => {
...
export async function runNpmAudit(target, config, addFinding, debugFn) {
  ...
  const out = await runToolAsync("npm", ["audit", "--json"], { cwd: target });
